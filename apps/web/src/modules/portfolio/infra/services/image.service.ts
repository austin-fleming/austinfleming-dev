...
const makeQuality = (quality: number) => `q=${quality}`;
const makeFormat = (format: ImageFormat) => `fm=${format}`;
const makeWidth = (width: number) => `w=${width}`;
const makeHeight = (height: number) => `h=${height}`;

const constructImage = (imageUrl: string, options: ImageOptions) =>
	imageUrl +
	'?' +
	[
		!!options.format && makeFormat(options.format),
		!!options.quality && makeQuality(options.quality),
		!!options.width && makeWidth(options.width),
		!!options.height && makeHeight(options.height),
		'fit=crop'
	]
